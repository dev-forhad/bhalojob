...
<div class="form-body">
    ...
    <div class="form-group {!! APFrmErrHelp::hasError($errors, 'category_id') !!}" id="category_id_div">
        {!! Form::label('category_id', 'Category', ['class' => 'bold']) !!}
        {!! Form::select('category_id', ['' => 'Select Category'] + $categories, null, [
            'class' => 'form-control',
            'id' => 'category_id',
        ]) !!}
        {!! APFrmErrHelp::showErrors($errors, 'category_id') !!}
    </div>


    <div class="form-group {!! APFrmErrHelp::hasError($errors, 'sub_category_id') !!}" id="sub_category_id_div">
        {!! Form::label('sub_category_id', 'Sub Category', ['class' => 'bold']) !!}
        <span id="default_sub_category_dd">
            {!! Form::select('sub_category_id', ['' => 'Select Sub Category'], null, ['class' => 'form-control', 'id' => 'sub_category_id']) !!}
        </span>
        {!! APFrmErrHelp::showErrors($errors, 'sub_category_id') !!}
    </div>
    ...
    <div class="form-group {!! APFrmErrHelp::hasError($errors, 'industry_id') !!}" id="industry_id_div">
        {!! Form::label('industry_id', 'Industry or Subject', ['class' => 'bold']) !!}
        <span id="default_industry_dd">
            {!! Form::select('industry_id', ['' => 'Select Industry'], null, ['class' => 'form-control', 'id' => 'industry_id']) !!}
        </span>
                ...
                $.post("{{ route('filter.default.sub.category.dropdown') }}", {
                    ...
                    .done(function(response) {
                       // console.log(response);
                        ...
                        filterDefaultIndustry(<?php echo old('city_id', isset($job) ? $job->city_id : 0); ?>);
                    });
            }
        }

        function filterDefaultIndustry(industry_id) {
            // company_id == industry_id when change al
            var sub_category_id = $('#sub_category_id').val();
           // alert(sub_category_id);
            if (sub_category_id != '') {
                $.post("{{ route('filter.default.industry.dropdown') }}", {
                    sub_category_id: sub_category_id,
                    ...
                    .done(function(response) {
                        console.log(response);
                        $('#default_industry_dd').html(response);
                    });
            }
        }
