{!! APFrmErrHelp::showErrorsNotice($errors) !!}
@include('flash::message')
<div class="form-body">
    {{-- facebook login --}}
    <div class="form-group {!! APFrmErrHelp::hasError($errors, 'facebook_app_id') !!}">
        {!! Form::label('facebook_app_id', 'Facebook App ID', ['class' => 'bold']) !!}
        {!! Form::text('facebook_app_id', null, [
            'class' => 'form-control',
            'id' => 'facebook_app_id',
            'placeholder' => 'Facebook App ID',
        ]) !!}
        {!! APFrmErrHelp::showErrors($errors, 'facebook_app_id') !!}
    </div>
    <div class="form-group {!! APFrmErrHelp::hasError($errors, 'facebeek_app_secret') !!}">
        {!! Form::label('facebeek_app_secret', 'Facebeek App Secret', ['class' => 'bold']) !!}
        {!! Form::text('facebeek_app_secret', null, [
            'class' => 'form-control',
            'id' => 'facebeek_app_secret',
            'placeholder' => 'Facebeek App Secret',
        ]) !!}
        {!! APFrmErrHelp::showErrors($errors, 'facebeek_app_secret') !!}
    </div>
    {{-- google login --}}
    <div class="form-group {!! APFrmErrHelp::hasError($errors, 'google_app_id') !!}">
        {!! Form::label('google_app_id', 'Google App ID', ['class' => 'bold']) !!}
        {!! Form::text('google_app_id', null, [
            'class' => 'form-control',
            'id' => 'google_app_id',
            'placeholder' => 'Google App ID',
        ]) !!}
        {!! APFrmErrHelp::showErrors($errors, 'google_app_id') !!}
    </div>
    <div class="form-group {!! APFrmErrHelp::hasError($errors, 'google_app_secret') !!}">
        {!! Form::label('google_app_secret', 'Google App Secret', ['class' => 'bold']) !!}
        {!! Form::text('google_app_secret', null, [
            'class' => 'form-control',
            'id' => 'google_app_secret',
            'placeholder' => 'Google App Secret',
        ]) !!}
        {!! APFrmErrHelp::showErrors($errors, 'google_app_secret') !!}
    </div>

    {{-- yahoo login --}}
    <div class="form-group {!! APFrmErrHelp::hasError($errors, 'yahoo_app_id') !!}">
        {!! Form::label('yahoo_app_id', 'Yahoo App ID', ['class' => 'bold']) !!}
        {!! Form::text('yahoo_app_id', null, [
            'class' => 'form-control',
            'id' => 'yahoo_app_id',
            'placeholder' => 'Yahoo App ID',
        ]) !!}
        {!! APFrmErrHelp::showErrors($errors, 'yahoo_app_id') !!}
    </div>
    <div class="form-group {!! APFrmErrHelp::hasError($errors, 'yahoo_app_secret') !!}">
        {!! Form::label('yahoo_app_secret', 'Yahoo App Secret', ['class' => 'bold']) !!}
        {!! Form::text('yahoo_app_secret', null, [
            'class' => 'form-control',
            'id' => 'yahoo_app_secret',
            'placeholder' => 'Yahoo App Secret',
        ]) !!}
        {!! APFrmErrHelp::showErrors($errors, 'yahoo_app_secret') !!}
    </div>
    {{-- linkedin login --}}
    <div class="form-group {!! APFrmErrHelp::hasError($errors, 'linkedin_app_id') !!}">
        {!! Form::label('linkedin_app_id', 'Linkedin App ID', ['class' => 'bold']) !!}
        {!! Form::text('linkedin_app_id', null, [
            'class' => 'form-control',
            'id' => 'linkedin_app_id',
            'placeholder' => 'Linkedin App ID',
        ]) !!}
        {!! APFrmErrHelp::showErrors($errors, 'linkedin_app_id') !!}
    </div>
    <div class="form-group {!! APFrmErrHelp::hasError($errors, 'linkedin_app_secret') !!}">
        {!! Form::label('linkedin_app_secret', 'Linkedin App Secret', ['class' => 'bold']) !!}
        {!! Form::text('linkedin_app_secret', null, [
            'class' => 'form-control',
            'id' => 'linkedin_app_secret',
            'placeholder' => 'Linkedin App Secret',
        ]) !!}
        {!! APFrmErrHelp::showErrors($errors, 'linkedin_app_secret') !!}
    </div>



    <div class="form-group {!! APFrmErrHelp::hasError($errors, 'twitter_app_id') !!}">
        {!! Form::label('twitter_app_id', 'Twitter App ID', ['class' => 'bold']) !!}
        {!! Form::text('twitter_app_id', null, [
            'class' => 'form-control',
            'id' => 'twitter_app_id',
            'placeholder' => 'Twitter App ID',
        ]) !!}
        {!! APFrmErrHelp::showErrors($errors, 'twitter_app_id') !!}
    </div>
    <div class="form-group {!! APFrmErrHelp::hasError($errors, 'twitter_app_secret') !!}">
        {!! Form::label('twitter_app_secret', 'Twitter App Secret', ['class' => 'bold']) !!}
        {!! Form::text('twitter_app_secret', null, [
            'class' => 'form-control',
            'id' => 'twitter_app_secret',
            'placeholder' => 'Twitter App Secret',
        ]) !!}
        {!! APFrmErrHelp::showErrors($errors, 'twitter_app_secret') !!}
    </div>
</div>
