...
$education = App\ProfileEducation::where('user_id', $user_id)->first();
$Experience = App\ProfileExperience::where('user_id', $user_id)->first();
$langauge = App\ProfileLanguage::where('user_id', $user_id)->first();
$user = App\User::where('id', $user_id)->first();
$certificate = App\ProfileSkill::where('user_id', $user_id)->first();
$pro_summery = App\ProfileSummary::where('user_id', $user_id)->first();
$experiences = App\ProfileExperience::where('user_id', $user_id)->get();
if($experiences){
...
// @dd($langauge); 
 ...
 function profilePercentageCalculatio($user, $education, $Experience, $langauge, $pro_summery)
 {    
    // $totalFields = 45;
    $totalFields = 32;
    // user/////////////////
    ...
    // $filledFields+=$user->has_children ? 1: 0;
    $filledFields+=$user->birth_year ? 1:0; 
    $filledFields+=$user->birth_month ? 1:0; 
    $filledFields+=$user->birth_day ? 1:0;
    
    $filledFields+= $user->country_id ? 1:0;
    
    $filledFields+= $user->nationality_id ? 1:0;
    $filledFields+= $user->phone ? 1:0;
    $filledFields+= $user->bd_postal_code ? 1:0; 
    $filledFields+= $user->bd_address ? 1:0; 
    $filledFields+= $user->bd_village ? 1:0;
    $filledFields+= $user->state_id ? 1:0;
    $filledFields+= $user->city_id ? 1:0;
    $filledFields+= $user->mother_tongue ? 1:0;
    $filledFields+= $user->image ? 1:0;
   ...
   // Education Profile/////////////////
    ...
    //   $filledFields+=$education->degree_level_id ? 1:0; 
    }
...
// Experience  Profile/////////////////
    // $exp = $Experience->is_currently_working ? 1:0; 
    ...
    $filledFields+=@$pro_summery->summary ? 1:0; 
    // $filledFields+=@$langauge->language_type_id ? 1:0; 
...
$percentage = $filledFields / $totalFields * 100;
 ...
 $total= intval(profilePercentageCalculatio($user, $education, $Experience, $langauge, $pro_summery));
...
<div class="progress">
  <div class="progress-bar bg-danger" role="progressbar" style="width: {{@$total}}%;" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100">{{@$total}}%</div>
    ...
    <ul class="usernavdash">
        ...
        <li class="{{ Request::url() == route('profilepage') ? 'active' : '' }}"><a href="{{ route('profilepage') }}"><i class="fas fa-pencil" aria-hidden="true"></i> {{__('Edit Profile')}}</a></li>
        <li><a href="{{ route('resume.user', Auth::user()->id) }}"><i class="fa fa-print" aria-hidden="true"></i> {{__('View Resume')}}</a></li>
