        ...
        <div class="page-content">
            ...
            <div class="page-bar">
                <ul class="page-breadcrumb">
                    ...
                    <li> <span>Success Job</span> </li>
                </ul>
            </div>
            <!-- END PAGE BAR -->
            <!-- BEGIN PAGE TITLE-->
            <h3 class="page-title">Manage Success Job <small>Success Job</small> </h3>
            <!-- END PAGE TITLE-->
            <!-- END PAGE HEADER-->
                    ...
                    <div class="portlet light portlet-fit portlet-datatable bordered">
                        <div class="portlet-title">
                            <div class="caption"> <i class="icon-settings font-dark"></i> <span
                                    class="caption-subject font-dark sbold uppercase">Success Job</span> </div>
                            <div class="actions"> <a href="{{ route('admin.successCreate') }}"
                                ...
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        @php
                                            $i = 1;
                                        @endphp
                                        @foreach ($allsuccessData as $each)
                                            <tr>
                                                <th>{{ $i++ }}</th>
                                                <td>{{ $each->name }}</td>
                                                <td>{{ $each->age }}</td>
                                                <td>{{ $each->gender }}</td>
                                                <td><img width="50px" src="{{ asset('successjob/' . $each->image) }}"
                                                        alt=""></td>
                                                ...
                                                <td>
                                                    <a href="{{ url('admin/successEdit/' . $each->id) }}"
                                                        class="btn btn-success btn-sm"><i class="fa fa-pencil" aria-hidden="true"></i> Edit>

                                                        <a onclick="return confirm('Are you sure you want to delete this item?');"   href="{{ url('admin/successDelete/' . $each->id) }}"
                                                        class="btn btn-danger btn-sm"><i class="fa fa-trash-o" aria-hidden="true"></i> Delete</a>
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
