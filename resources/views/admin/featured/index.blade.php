...
@section('content')
    ...
    <div class="page-content-wrapper">
        ...
        <div class="page-content">
            ...
            <!-- END PAGE BAR -->
            <!-- BEGIN PAGE TITLE-->
            <h3 class="page-title">Manage Featured <small>Featured</small> </h3>
            <!-- END PAGE TITLE-->
            <!-- END PAGE HEADER-->
                    ...
                    <div class="portlet light portlet-fit portlet-datatable bordered">
                        <div class="portlet-title">
                            <div class="caption"> <i class="icon-settings font-dark"></i> <span
                                    class="caption-subject font-dark sbold uppercase">Featured</span> </div>
                            <div class="actions"> <a href="{{ route('admin.fiCreate') }}" class="btn btn-xs btn-success"><i
                                ...
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        @php
                                            $i = 1;
                                        @endphp
                                        @foreach ($alljobservice as $each)
                                            <tr>
                                                <th>{{ $i++ }}</th>
                                                <td>
                                                    @if ($each->institute == 1)
                                                        {{ 'Top Companies' }}
                                                    @elseif($each->institute == 2)
                                                        {{ 'Top Colleges' }}
                                                    @else
                                                ...
                                                <td>{{ $each->name }}</td>
                                                <td><img width="50px" src="{{ asset('/featured/' . $each->logo) }}"
                                                        alt="">
                                                </td>
                                                <td>
                                                    <a href="{{ url('admin/fiEdit/' . $each->id) }}"     class="btn btn-success btn-sm"><span class="fa fa-pencil"></span>Edit</a>
                                                    <a onclick="return confirm('Are you sure you want to delete this item?');"   href="{{ url('admin/jcDelete/' . $each->id) }}"
