...
@php
    ...
    $certificateInfos = App\ProfileSkill::where("user_id",$user->id)->whereNotNull('lang_id')->get();
    $drivingInfos = App\ProfileSkill::where("user_id",$user->id)->whereNotNull('driving_class_id')->get();
        ...
        <div class="row">
            <div class="col-md-12">
                <table class="table table-bordered">
                   ...
                   // dd($cerinfos);
                    ...
                    @endphp
                                                ...
                                                <td>
                                                    <select class="form-control"  required>
                                                        ...
                                                        @foreach($years as $year)
                                                            <option value="{{$year->id}}" <?php if(!empty($certificate->pass_year) && $certificate->pass_year == $year->id): echo "selected";endif; ?> >{{$year->year_value}}</option>
                                                        @endforeach
                                                    </select>
                                                </td>
                                                <td>
                                                    <select class="form-control select2"  required>
                                                        ...
                                                        @foreach($months as $month)
                                                            <option value="{{$month->id}}" <?php if(!empty($certificate->pass_month) && $certificate->pass_month == $month->id): echo "selected";endif; ?> >{{$month->eng_title}}</option>
                                                        @endforeach
                                                    </select>
                                                </td>
                                            ...
                                            <tr>
                                                <td>
                                                    <select class="form-control"  name="other_certification[<?php echo $certificate->slug;?>][pass_year]"  required>
                                                        <option value="" disabled>Year</option>
                                                        @foreach($years as $year)
                                                            <option value="{{$year->id}}" <?php if(!empty($certificate->pass_year) && $certificate->pass_year == $year->id): echo "selected";endif; ?> >{{$year->jp_year_value}}</option>
                                                        @endforeach
                                                    </select>
                                                </td>
                                                <td>
                                                    <select class="form-control"  name="other_certification[<?php echo $certificate->slug;?>][pass_month]" required>
                                                        ...
                                                        @foreach($months as $month)
                                                            <option value="{{$month->id}}" <?php if(!empty($certificate->pass_month) && $certificate->pass_month == $month->id): echo "selected";endif; ?> >{{$month->jp_title}}</option>
                                                        @endforeach
                                                    </select>
                                                </td>
                            ...
                            <tr>
                                ...
                                <td> {{$certificate->certificate_title}} {{ $certificate->getLang('lang') }} Certificate</td>
                 ...
                 @endforeach
                 @if (isset($drivingInfos) && count($drivingInfos)):
                  @foreach($drivingInfos as $key =>  $certificate)
                   <tr>
                                <td colspan="3" style="background-color: #f1f1f1; color:red;text-align: center;"> Certificate No: @php echo $key+1; @endphp </td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td> {{$certificate->certificate_title}}Driving Lincense </td>
