            ...
            <div class="col-md-12">
                <table class="table table-bordered">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colspan="5" style="background-color: #f1f1f1; color:red;text-align: center;">{{ $education->degreeLevel->degree_level??'N/A' }}</td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <select class="form-control select2"    >
                                    ...
                                    @foreach($years as $year)
                                        <option value="{{$year->id}}" <?php if(!empty($education->entrance_year) && $education->entrance_year == $year->id): echo "selected";endif; ?> >{{$year->year_value}}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td>
                                <select class="form-control"  >
                                    ...
                                    @foreach($months as $month)
                                        <option value="{{$month->id}}" <?php if(!empty($education->entrance_month) && $education->entrance_month == $month->id): echo "selected";endif; ?> >{{$month->eng_title}}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td>
                                <select class="form-control"     name="education_history[{{$education->id}}][entrance_year]" required>
                                    <option value="" disabled>Year</option>
                                    @foreach($years as $year)
                                        <option value="{{$year->id}}" <?php if(!empty($education->entrance_year) && $education->entrance_year == $year->id): echo "selected";endif; ?> >{{$year->jp_year_value}}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td>
                                <select class="form-control"  name="education_history[{{$education->id}}][entrance_month]" required>
                                    <option value="" disabled>Month</option>
                                    @foreach($months as $month)
                                        <option value="{{$month->id}}"  <?php if(!empty($education->entrance_month) && $education->entrance_month == $month->id): echo "selected";endif; ?> >{{$month->jp_title}}</option>
                                    @endforeach
                                </select>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <select class="form-control select2"    >
                                    <option value="" disabled>Year</option>
                                    @foreach($years as $year)
                                        <option value="{{$year->id}}" <?php if(!empty($education->pass_year) && $education->pass_year == $year->id): echo "selected";endif; ?> >{{$year->year_value}}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td>
                                <select class="form-control"  >
                                    <option value="" disabled>Month</option>
                                    @foreach($months as $month)
                                        <option value="{{$month->id}}" <?php if(!empty($education->pass_month) && $education->pass_month == $month->id): echo "selected";endif; ?> >{{$month->eng_title}}</option>
                                    @endforeach
                                </select>
                            </td>
                       
                            <td>
                                <select class="form-control"     name="education_history[{{$education->id}}][pass_year]" required>
                                    <option value="" disabled>Year</option>
                                    @foreach($years as $year)
                                        <option value="{{$year->id}}" <?php if(!empty($education->pass_year) && $education->pass_year == $year->id): echo "selected";endif; ?> >{{$year->jp_year_value}}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td>
                                <select class="form-control"  name="education_history[{{$education->id}}][pass_month]" required>
                                    <option value="" disabled>Month</option>
                                    @foreach($months as $month)
                                        <option value="{{$month->id}}"  <?php if(!empty($education->pass_month) && $education->pass_month == $month->id): echo "selected";endif; ?> >{{$month->jp_title}}</option>
                                    @endforeach
                                </select>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td colspan="2"> {{$education->institution_name}} </td>
                            <td colspan="2"><input type="text" name="education_history[{{$education->id}}][institution_name]" value="{{isset($eduInfo) ? collect($eduInfo)[$education->id]->institution_name : ''}}" class="form-control"></td>
                        </tr>
                        @if($education->major)
                            <tr>
                                <td>Major / Subject (IF Have)</td>
                                <td colspan="2"> {{$education->major}} </td>
                                <td colspan="2"><input type="text" name="education_history[{{$education->id}}][major]" value="{{isset($eduInfo) ? collect($eduInfo)[$education->id]->major : ''}}" class="form-control"></td>
                            </tr>
                        @endif
                        @endforeach
                    </tbody>
